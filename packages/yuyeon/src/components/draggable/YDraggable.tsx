  ...
  setup(props, { slots, emit }) {
    ...
    const handleEl = computed(() => {
      ...
      if (typeof props.handle === 'string') {
        ...
        if (hEl?.nodeType === 1) el = hEl as HTMLElement;
      }
      return el;
    });
    ...
    const start = (e: PointerEvent) => {
      ...
      if (!targetEl.value || !containerEl.value) return;
      ...
      const pos = {
        x:
          ...
          (containerEl.value
            ? targetRect.left -
              containerRect!.left +
              containerEl.value.scrollLeft
            : targetRect.left),
        y:
          e.clientY -
          (containerEl.value
            ? targetRect.top - containerRect!.top + containerEl.value.scrollTop
            : targetRect.top),
      };
